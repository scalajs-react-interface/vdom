package sri.web.vdom

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.ScalaJSDefined

@ScalaJSDefined
trait HTMLProps extends js.Object {
  val defer: js.UndefOr[Boolean] = js.undefined
  val coords: js.UndefOr[String] = js.undefined
  val span: js.UndefOr[Int] = js.undefined
  val method: js.UndefOr[String] = js.undefined
  val keyType: js.UndefOr[String] = js.undefined
  val challenge: js.UndefOr[String] = js.undefined
  val name: js.UndefOr[String] = js.undefined
  val formNoValidate: js.UndefOr[Boolean] = js.undefined
  val tabIndex: js.UndefOr[Int] = js.undefined
  val is: js.UndefOr[String] = js.undefined
  val accept: js.UndefOr[String] = js.undefined
  val loop: js.UndefOr[Boolean] = js.undefined
  val marginWidth: js.UndefOr[String] = js.undefined
  val step: js.UndefOr[String] = js.undefined
  val classID: js.UndefOr[String] = js.undefined
  val size: js.UndefOr[Int] = js.undefined
  val optimum: js.UndefOr[Double] = js.undefined
  val readOnly: js.UndefOr[Boolean] = js.undefined
  val pattern: js.UndefOr[String] = js.undefined
  val scrolling: js.UndefOr[String] = js.undefined
  val nonce: js.UndefOr[String] = js.undefined
  val cellSpacing: js.UndefOr[String] = js.undefined
  val frameBorder: js.UndefOr[String] = js.undefined
  val contentEditable: js.UndefOr[String] = js.undefined
  val role: js.UndefOr[String] = js.undefined
  val checked: js.UndefOr[String] = js.undefined
  val acceptCharset: js.UndefOr[String] = js.undefined
  val scope: js.UndefOr[String] = js.undefined
  val icon: js.UndefOr[String] = js.undefined
  val hrefLang: js.UndefOr[String] = js.undefined
  val data: js.UndefOr[String] = js.undefined
  val label: js.UndefOr[String] = js.undefined
  val hidden: js.UndefOr[Boolean] = js.undefined
  val marginHeight: js.UndefOr[String] = js.undefined
  val rel: js.UndefOr[String] = js.undefined
  val height: js.UndefOr[Double] = js.undefined
  val radioGroup: js.UndefOr[String] = js.undefined
  val min: js.UndefOr[Double] = js.undefined
  val selected: js.UndefOr[Boolean] = js.undefined
  val cellPadding: js.UndefOr[String] = js.undefined
  val srcDoc: js.UndefOr[String] = js.undefined
  val charSet: js.UndefOr[String] = js.undefined
  val placeholder: js.UndefOr[String] = js.undefined
  val open: js.UndefOr[Boolean] = js.undefined
  val inputMode: js.UndefOr[String] = js.undefined
  val dateTime: js.UndefOr[String] = js.undefined
  val crossOrigin: js.UndefOr[String] = js.undefined
  val autoFocus: js.UndefOr[Boolean] = js.undefined
  val formTarget: js.UndefOr[String] = js.undefined
  val poster: js.UndefOr[String] = js.undefined
  val htmlFor: js.UndefOr[String] = js.undefined
  val seamless: js.UndefOr[String] = js.undefined
  val dir: js.UndefOr[String] = js.undefined
  val low: js.UndefOr[Double] = js.undefined
  val id: js.UndefOr[String] = js.undefined
  val async: js.UndefOr[Boolean] = js.undefined
  val wrap: js.UndefOr[String] = js.undefined
  val autoComplete: js.UndefOr[String] = js.undefined
  val max: js.UndefOr[Double] = js.undefined
  val profile: js.UndefOr[String] = js.undefined
  val allowFullScreen: js.UndefOr[Boolean] = js.undefined
  val cite: js.UndefOr[String] = js.undefined
  val draggable: js.UndefOr[Boolean] = js.undefined
  val alt: js.UndefOr[String] = js.undefined
  val httpEquiv: js.UndefOr[String] = js.undefined
  val formMethod: js.UndefOr[String] = js.undefined
  val default: js.UndefOr[Boolean] = js.undefined
  val accessKey: js.UndefOr[String] = js.undefined
  val className: js.UndefOr[String] = js.undefined
  val srcLang: js.UndefOr[String] = js.undefined
  val sandbox: js.UndefOr[String] = js.undefined
  val src: js.UndefOr[String] = js.undefined
  val encType: js.UndefOr[String] = js.undefined
  val colSpan: js.UndefOr[Int] = js.undefined
  val content: js.UndefOr[String] = js.undefined
  val mediaGroup: js.UndefOr[String] = js.undefined
  val rowSpan: js.UndefOr[Int] = js.undefined
  val media: js.UndefOr[String] = js.undefined
  val useMap: js.UndefOr[String] = js.undefined
  val scoped: js.UndefOr[String] = js.undefined
  val disabled: js.UndefOr[Boolean] = js.undefined
  val target: js.UndefOr[String] = js.undefined
  val srcSet: js.UndefOr[String] = js.undefined
  val minLength: js.UndefOr[Int] = js.undefined
  val spellCheck: js.UndefOr[Boolean] = js.undefined
  val shape: js.UndefOr[String] = js.undefined
  val formEncType: js.UndefOr[String] = js.undefined
  val sizes: js.UndefOr[String] = js.undefined
  val manifest: js.UndefOr[String] = js.undefined
  val multiple: js.UndefOr[Boolean] = js.undefined
  val kind: js.UndefOr[String] = js.undefined
  val title: js.UndefOr[String] = js.undefined
  val allowTransparency: js.UndefOr[Boolean] = js.undefined
  val lang: js.UndefOr[String] = js.undefined
  val headers: js.UndefOr[String] = js.undefined
  val `type`: js.UndefOr[String] = js.undefined
  val form: js.UndefOr[String] = js.undefined
  val required: js.UndefOr[Boolean] = js.undefined
  val href: js.UndefOr[String] = js.undefined
  val start: js.UndefOr[Int] = js.undefined
  val capture: js.UndefOr[Boolean] = js.undefined
  val preload: js.UndefOr[String] = js.undefined
  val rows: js.UndefOr[Int] = js.undefined
  val width: js.UndefOr[Double] = js.undefined
  val autoPlay: js.UndefOr[Boolean] = js.undefined
  val reversed: js.UndefOr[Boolean] = js.undefined
  val download: js.UndefOr[String] = js.undefined
  val value: js.UndefOr[String | Double | Int] = js.undefined
  val muted: js.UndefOr[Boolean] = js.undefined
  val noValidate: js.UndefOr[Boolean] = js.undefined
  val maxLength: js.UndefOr[Int] = js.undefined
  val action: js.UndefOr[String] = js.undefined
  val summary: js.UndefOr[String] = js.undefined
  val integrity: js.UndefOr[String] = js.undefined
  val contextMenu: js.UndefOr[String] = js.undefined
  val high: js.UndefOr[Double] = js.undefined
  val cols: js.UndefOr[Int] = js.undefined
  val formAction: js.UndefOr[String] = js.undefined
  val list: js.UndefOr[String] = js.undefined
  val keyParams: js.UndefOr[String] = js.undefined
  val controls: js.UndefOr[Boolean] = js.undefined
}

@ScalaJSDefined
trait SVGProps extends js.Object {
  val amplitude: js.UndefOr[Double] = js.undefined
  val numOctaves: js.UndefOr[Int] = js.undefined
  val colorInterpolation: js.UndefOr[String] = js.undefined
  val format: js.UndefOr[String] = js.undefined
  val divisor: js.UndefOr[Double] = js.undefined
  val kernelUnitLength: js.UndefOr[Double] = js.undefined
  val floodColor: js.UndefOr[String] = js.undefined
  val clipPathUnits: js.UndefOr[String] = js.undefined
  val x: js.UndefOr[String | Double] = js.undefined
  val contentStyleType: js.UndefOr[String] = js.undefined
  val begin: js.UndefOr[String] = js.undefined
  val intercept: js.UndefOr[String] = js.undefined
  val xlinkRole: js.UndefOr[String] = js.undefined
  val maskUnits: js.UndefOr[String] = js.undefined
  val accumulate: js.UndefOr[String] = js.undefined
  val contentScriptType: js.UndefOr[String] = js.undefined
  val k2: js.UndefOr[Double] = js.undefined
  val in: js.UndefOr[String] = js.undefined
  val textRendering: js.UndefOr[String] = js.undefined
  val kernelMatrix: js.UndefOr[js.Array[Double]] = js.undefined
  val keyTimes: js.UndefOr[String] = js.undefined
  val shapeRendering: js.UndefOr[String] = js.undefined
  val dy: js.UndefOr[Double] = js.undefined
  val mask: js.UndefOr[String] = js.undefined
  val pointsAtZ: js.UndefOr[Double] = js.undefined
  val seed: js.UndefOr[Int] = js.undefined
  val tableValues: js.UndefOr[String] = js.undefined
  val overflow: js.UndefOr[String] = js.undefined
  val xlinkShow: js.UndefOr[String] = js.undefined
  val strokeDasharray: js.UndefOr[String] = js.undefined
  val maskContentUnits: js.UndefOr[String] = js.undefined
  val textAnchor: js.UndefOr[String] = js.undefined
  val arabicForm: js.UndefOr[String] = js.undefined
  val xlinkActuate: js.UndefOr[String] = js.undefined
  val keyPoints: js.UndefOr[String] = js.undefined
  val y: js.UndefOr[String | Double] = js.undefined
  val strokeOpacity: js.UndefOr[String | Double] = js.undefined
  val refY: js.UndefOr[Double] = js.undefined
  val viewBox: js.UndefOr[String] = js.undefined
  val patternTransform: js.UndefOr[String] = js.undefined
  val cx: js.UndefOr[String | Double] = js.undefined
  val textDecoration: js.UndefOr[String] = js.undefined
  val scale: js.UndefOr[Double] = js.undefined
  val filterUnits: js.UndefOr[String] = js.undefined
  val externalResourcesRequired: js.UndefOr[Double] = js.undefined
  val baseFrequency: js.UndefOr[Double] = js.undefined
  val g2: js.UndefOr[String] = js.undefined
  val stroke: js.UndefOr[String] = js.undefined
  val clipPath: js.UndefOr[String] = js.undefined
  val result: js.UndefOr[String] = js.undefined
  val specularExponent: js.UndefOr[Double] = js.undefined
  val fillOpacity: js.UndefOr[String | Double] = js.undefined
  val horizOriginY: js.UndefOr[Double] = js.undefined
  val fontSizeAdjust: js.UndefOr[String | Double] = js.undefined
  val clipRule: js.UndefOr[String] = js.undefined
  val limitingConeAngle: js.UndefOr[Double] = js.undefined
  val fx: js.UndefOr[Double] = js.undefined
  val additive: js.UndefOr[String] = js.undefined
  val local: js.UndefOr[String] = js.undefined
  val paintOrder: js.UndefOr[String] = js.undefined
  val stopOpacity: js.UndefOr[String | Double] = js.undefined
  val colorInterpolationFilters: js.UndefOr[String] = js.undefined
  val calcMode: js.UndefOr[String] = js.undefined
  val refX: js.UndefOr[Double] = js.undefined
  val pointsAtY: js.UndefOr[Double] = js.undefined
  val repeatDur: js.UndefOr[String] = js.undefined
  val dur: js.UndefOr[String | Double] = js.undefined
  val fontStyle: js.UndefOr[String] = js.undefined
  val in2: js.UndefOr[String] = js.undefined
  val direction: js.UndefOr[String] = js.undefined
  val enableBackground: js.UndefOr[String] = js.undefined
  val pathLength: js.UndefOr[Double] = js.undefined
  val colorRendering: js.UndefOr[String] = js.undefined
  val g1: js.UndefOr[String] = js.undefined
  val rx: js.UndefOr[String | Double] = js.undefined
  val specularConstant: js.UndefOr[Int] = js.undefined
  val x2: js.UndefOr[String | Double] = js.undefined
  val viewTarget: js.UndefOr[String] = js.undefined
  val exponent: js.UndefOr[Double] = js.undefined
  val cursor: js.UndefOr[String] = js.undefined
  val radius: js.UndefOr[String | Double] = js.undefined
  val yChannelSelector: js.UndefOr[String] = js.undefined
  val cy: js.UndefOr[String | Double] = js.undefined
  val xmlBase: js.UndefOr[String] = js.undefined
  val to: js.UndefOr[String | Double] = js.undefined
  val attributeName: js.UndefOr[String] = js.undefined
  val glyphRef: js.UndefOr[String] = js.undefined
  val xmlSpace: js.UndefOr[String] = js.undefined
  val filter: js.UndefOr[String] = js.undefined
  val rotate: js.UndefOr[String | Double] = js.undefined
  val lengthAdjust: js.UndefOr[String] = js.undefined
  val repeatCount: js.UndefOr[String | Int] = js.undefined
  val gradientTransform: js.UndefOr[String] = js.undefined
  val fy: js.UndefOr[Double] = js.undefined
  val version: js.UndefOr[String] = js.undefined
  val xlinkHref: js.UndefOr[String] = js.undefined
  val markerWidth: js.UndefOr[String] = js.undefined
  val fontFamily: js.UndefOr[String] = js.undefined
  val gradientUnits: js.UndefOr[String] = js.undefined
  val points: js.UndefOr[String] = js.undefined
  val pointsAtX: js.UndefOr[Double] = js.undefined
  val kerning: js.UndefOr[String] = js.undefined
  val offset: js.UndefOr[String | Double] = js.undefined
  val strokeDashoffset: js.UndefOr[String] = js.undefined
  val horizOriginX: js.UndefOr[Double] = js.undefined
  val glyphOrientationHorizontal: js.UndefOr[String] = js.undefined
  val elevation: js.UndefOr[Double] = js.undefined
  val letterSpacing: js.UndefOr[String] = js.undefined
  val preserveAspectRatio: js.UndefOr[Boolean] = js.undefined
  val strokeLinecap: js.UndefOr[String] = js.undefined
  val y1: js.UndefOr[String | Double] = js.undefined
  val baseProfile: js.UndefOr[String] = js.undefined
  val horizAdvX: js.UndefOr[Double] = js.undefined
  val wordSpacing: js.UndefOr[String | Double] = js.undefined
  val xmlLang: js.UndefOr[String] = js.undefined
  val xChannelSelector: js.UndefOr[String] = js.undefined
  val order: js.UndefOr[Int] = js.undefined
  val fill: js.UndefOr[String] = js.undefined
  val systemLanguage: js.UndefOr[Boolean] = js.undefined
  val operator: js.UndefOr[String] = js.undefined
  val by: js.UndefOr[Double] = js.undefined
  val origin: js.UndefOr[String] = js.undefined
  val markerUnits: js.UndefOr[String] = js.undefined
  val markerHeight: js.UndefOr[String] = js.undefined
  val pointerEvents: js.UndefOr[String] = js.undefined
  val floodOpacity: js.UndefOr[String | Double] = js.undefined
  val writingMode: js.UndefOr[String | Double] = js.undefined
  val patternContentUnits: js.UndefOr[String] = js.undefined
  val xlinkType: js.UndefOr[String] = js.undefined
  val xlinkArcrole: js.UndefOr[String] = js.undefined
  val from: js.UndefOr[Double] = js.undefined
  val preserveAlpha: js.UndefOr[Boolean] = js.undefined
  val fontVariant: js.UndefOr[String] = js.undefined
  val requiredExtensions: js.UndefOr[String] = js.undefined
  val baselineShift: js.UndefOr[String] = js.undefined
  val bias: js.UndefOr[Double] = js.undefined
  val requiredFeatures: js.UndefOr[String] = js.undefined
  val fontSize: js.UndefOr[String | Double] = js.undefined
  val ry: js.UndefOr[String | Double] = js.undefined
  val mode: js.UndefOr[String] = js.undefined
  val y2: js.UndefOr[String | Double] = js.undefined
  val edgeMode: js.UndefOr[String] = js.undefined
  val markerMid: js.UndefOr[String] = js.undefined
  val startOffset: js.UndefOr[String] = js.undefined
  val strokeMiterlimit: js.UndefOr[String] = js.undefined
  val x1: js.UndefOr[String | Double] = js.undefined
  val end: js.UndefOr[Double] = js.undefined
  val values: js.UndefOr[String] = js.undefined
  val attributeType: js.UndefOr[String] = js.undefined
  val k1: js.UndefOr[Double] = js.undefined
  val targetY: js.UndefOr[Double] = js.undefined
  val glyphOrientationVertical: js.UndefOr[String] = js.undefined
  val keySplines: js.UndefOr[String] = js.undefined
  val opacity: js.UndefOr[String] = js.undefined
  val markerStart: js.UndefOr[String] = js.undefined
  val primitiveUnits: js.UndefOr[String] = js.undefined
  val alignmentBaseline: js.UndefOr[String] = js.undefined
  val patternUnits: js.UndefOr[String] = js.undefined
  val surfaceScale: js.UndefOr[Double] = js.undefined
  val k4: js.UndefOr[Double] = js.undefined
  val dx: js.UndefOr[Double] = js.undefined
  val allowReorder: js.UndefOr[Boolean] = js.undefined
  val r: js.UndefOr[String | Double] = js.undefined
  val clip: js.UndefOr[String] = js.undefined
  val k3: js.UndefOr[Double] = js.undefined
  val restart: js.UndefOr[String] = js.undefined
  val fontStretch: js.UndefOr[String] = js.undefined
  val imageRendering: js.UndefOr[String] = js.undefined
  val stdDeviation: js.UndefOr[String | Double] = js.undefined
  val xlinkTitle: js.UndefOr[String] = js.undefined
  val colorProfile: js.UndefOr[String] = js.undefined
  val spacing: js.UndefOr[String] = js.undefined
  val stopColor: js.UndefOr[String] = js.undefined
  val strokeLinejoin: js.UndefOr[String] = js.undefined
  val lightingColor: js.UndefOr[String] = js.undefined
  val transform: js.UndefOr[String] = js.undefined
  val fontWeight: js.UndefOr[String] = js.undefined
  val diffuseConstant: js.UndefOr[Double] = js.undefined
  val visibility: js.UndefOr[String] = js.undefined
  val unicodeBidi: js.UndefOr[String] = js.undefined
  val targetX: js.UndefOr[Double] = js.undefined
  val strokeWidth: js.UndefOr[String | Double] = js.undefined
  val dominantBaseline: js.UndefOr[String] = js.undefined
  val textLength: js.UndefOr[String | Double] = js.undefined
  val slope: js.UndefOr[String | Double] = js.undefined
  val azimuth: js.UndefOr[Double] = js.undefined
  val zoomAndPan: js.UndefOr[String] = js.undefined
  val spreadMethod: js.UndefOr[String] = js.undefined
  val markerEnd: js.UndefOr[String] = js.undefined
  val z: js.UndefOr[String | Double] = js.undefined
  val display: js.UndefOr[String] = js.undefined
  val fillRule: js.UndefOr[String] = js.undefined
  val stitchTiles: js.UndefOr[String] = js.undefined
  val d: js.UndefOr[String] = js.undefined
}

@ScalaJSDefined
trait EventProps extends js.Object {
  var onTimeUpdate: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onAnimationStart: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onKeyUp: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDrag: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCompositionStart: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onAnimationIteration: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onCanPlayThrough: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onDragStart: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onLoadStart: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onMouseUp: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onKeyPress: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onError: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDragExit: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onSelect: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCompositionEnd: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onTouchCancel: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onTouchEnd: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onTransitionEnd: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onSeeking: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onPlay: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDoubleClick: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDragOver: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onEnded: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onEmptied: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onMouseEnter: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDragLeave: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCompositionUpdate: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onMouseLeave: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onPlaying: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onMouseDown: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onSuspend: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onEncrypted: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onProgress: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDurationChange: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onFocus: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCut: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onSeeked: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onLoadedData: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onStalled: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCanPlay: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onKeyDown: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onTouchStart: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onBlur: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onAbort: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onChange: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onVolumeChange: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onSubmit: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onLoadedMetadata: js.UndefOr[(_ <: SyntheticEvent[_]) => _] =
    js.undefined
  var onContextMenu: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onMouseMove: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onScroll: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onRateChange: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onAnimationEnd: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onWaiting: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onCopy: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDragEnd: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onWheel: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onPause: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDragEnter: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onClick: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onTouchMove: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onDrop: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onLoad: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onInput: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
  var onPaste: js.UndefOr[(_ <: SyntheticEvent[_]) => _] = js.undefined
}

@ScalaJSDefined
trait DOMProps extends HTMLProps with SVGProps with EventProps
